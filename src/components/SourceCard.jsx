  ...
  return (
    <div className="bg-[#111827] border border-slate-800 rounded-xl p-4 shadow-xl space-y-3 transition-all duration-200 hover:border-slate-700">
      <div className="flex items-center justify-between border-b border-slate-800 pb-2">
        ...
        <span className="text-[10px] font-mono text-indigo-300 bg-[#090d16] px-2 py-0.5 rounded border border-indigo-500/30 font-semibold">
          Air-Gapped Index
        </span>
      </div>
        ...
        {sourcesUsed.map((source) => (
          ...
          >
            <div className="flex items-center justify-between">
              <span className="font-semibold text-white truncate max-w-[180px]" title={source.title}>
                📄 {source.title}
              </span>
              <span className="text-[10px] font-mono bg-indigo-950/80 text-indigo-300 px-1.5 py-0.5 rounded font-semibold border border-indigo-500/30">
                {source.score}
              </span>
            </div>
